...
def _send(msg: MIMEMultipart) -> None:
    with smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT) as smtp:
        ...
        smtp.sendmail(settings.SMTP_EMAIL, msg["To"], msg.as_string())


def send_invite_email(*, recipient_email: str, therapist_name: str, invite_url: str) -> None:
    """Sends a patient invitation email. Raises RuntimeError if SMTP is not configured."""
      ...
      <table width="520" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="background:#0d2525;padding:28px 32px;text-align:center;">
            <h1 style="color:#4ecdc4;margin:0;font-size:1.6rem;letter-spacing:1px;">Nirbaan</h1>
            <p style="color:#8aa8a8;margin:4px 0 0;font-size:0.85rem;">AI-Powered Therapy Platform</p>
          </td>
        </tr>
        <tr>
          <td style="padding:32px;">
            <h2 style="color:#e0e0e0;margin:0 0 16px;">You have been invited</h2>
            <p style="color:#b0c8c8;line-height:1.6;margin:0 0 24px;">
              Your therapist <strong style="color:#4ecdc4;">{therapist_name}</strong>
              has invited you to create your patient account on Nirbaan.
            </p>
...
def send_ep_invite_email(*, recipient_email: str, therapist_name: str, invite_url: str) -> None:
    """Sends an emergency personnel invitation email. Raises RuntimeError if SMTP is not configured."""
    _check_smtp_configured()

    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Nirbaan emergency personnel invitation from {therapist_name}"
    msg["From"]    = settings.SMTP_EMAIL
    msg["To"]      = recipient_email

    plain = f"""\
You have been invited to join Nirbaan as emergency personnel by {therapist_name}.
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr><td align="center" style="padding:40px 16px;">
      <table width="520" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px;">
            <h2 style="color:#e0e0e0;margin:0 0 16px;">Emergency Personnel Invitation</h2>
            <p style="color:#b0c8c8;line-height:1.6;margin:0 0 24px;">
              <strong style="color:#4ecdc4;">{therapist_name}</strong>
              has invited you to register as emergency support personnel on Nirbaan,
              where you can coordinate with therapists and assist patients in crisis situations.
            </p>
            <div style="text-align:center;margin:0 0 28px;">
              <a href="{invite_url}"
                 style="display:inline-block;background:#4ecdc4;color:#0a1f1f;
