...
Nirbaan addresses a persistent infrastructure gap in digital mental health research: the absence of open-source, production-grade platforms that integrate structured clinical workflow management with supervised AI assistance for OCD treatment. Existing tools either provide passive symptom logging without therapeutic interaction, or deploy general-purpose language model chatbots without a clinical state model, therapist-scoped knowledge isolation, or human escalation controls. Nirbaan fills this gap by providing a fully containerised, reproducible reference implementation that researchers can deploy, extend, and evaluate without rebuilding foundational infrastructure.

For clinical informatics researchers, the platform generates structured longitudinal data --- including SUDS time-series from live ERP sessions, daily self-monitoring logs, fear ladder progression records, and AI-generated session reports --- within a single auditable environment. This data infrastructure directly supports empirical research into ERP adherence patterns, between-session symptom trajectories, and attrition factors in digital mental health delivery \cite{simpson2011, linardon2020}, domains where structured collection pipelines have been a persistent methodological barrier. The platform's hidden symptom detector further enables studies that evaluate how closely automated semantic audits of self-monitoring logs mirror expert-driven clinical observation.

For AI systems researchers, Nirbaan provides three engineering patterns that are underrepresented in open-source mental health software: stateful multi-agent routing with a deterministic clinical safety exit implemented as a LangGraph state graph; per-tenant-scoped retrieval-augmented generation using pgvector metadata filtering to isolate therapist workspaces; and PostgreSQL-checkpointed human-in-the-loop approval gates for AI-generated clinical content. The last pattern is practically significant: checkpointing preserves the therapist review state across server restarts during multi-day approval cycles, a fault-tolerance requirement absent from most published HiTL implementations. The federated QLoRA pipeline --- implemented using Flower with a FedAvg aggregation strategy over a Non-IID OCD dataset partitioned across four simulated clients --- provides a reproducible baseline for privacy-preserving language model specialisation in settings where centralising therapy data is ethically or legally constrained under regulations such as HIPAA and GDPR. The localised Ollama deployment demonstrates that clinical-grade generative models can operate entirely on-premise on consumer hardware, lowering operational costs for resource-constrained healthcare systems.

The platform architecture is extensible beyond OCD. The LangGraph routing system, Celery processing pipeline, therapist-scoped RAG isolation, and human escalation pathway are independent of OCD-specific content. Adapting Nirbaan to adjacent exposure-based therapies --- including anxiety disorders, PTSD, or specific phobias --- requires modifications to clinical content and taxonomy rather than architectural redesign, reducing the engineering barrier for researchers in neighbouring digital mental health domains.

Critically, therapist oversight in Nirbaan is an architectural constraint rather than a configurable option. AI-generated imaginal exposure scripts are withheld from patients until explicit therapist approval; knowledge retrieval is bounded by therapist-uploaded material; and human escalation pathways route urgent cases to human responders rather than to an AI fallback. These deterministic controls distinguish Nirbaan from autonomous LLM applications in healthcare and position it as responsible AI infrastructure for therapist-supervised research deployments. Accordingly, Nirbaan contributes not as a clinically validated treatment system, but as an open-source software foundation for prototyping and evaluating human-supervised, AI-assisted therapy workflows.
