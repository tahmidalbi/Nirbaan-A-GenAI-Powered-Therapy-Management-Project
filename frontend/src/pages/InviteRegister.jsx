...
import { validateInvitation, registerViaInvitation } from '../api/patient.api';
...
const InviteRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await registerViaInvitation(token, payload);
      setSuccess(true);
    } catch (err) {
      ...
      <div style={styles.center}>
        <div style={styles.card}>
          ...
          <p style={styles.muted}>
            Your account has been successfully linked to your therapist's workspace.
          </p>
          <button style={styles.btn} onClick={() => navigate('/patient/login')}>
            Go to Patient Login
          </button>
        </div>
      </div>
  ...
  return (
    <div style={styles.center}>
      <div style={styles.card}>
        <h2 style={styles.heading}>Create Your Patient Account</h2>
        <p style={styles.muted}>
          You have been invited by <strong style={{ color: '#4ecdc4' }}>{invite.therapist_name}</strong>.
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Field label="Conditions *">
            <input style={styles.input} name="conditions" value={formData.conditions} onChange={handleChange} required placeholder="e.g., OCD, Anxiety" />
          </Field>
          <Field label="Conditions Description">
            <textarea style={{ ...styles.input, resize: 'vertical' }} name="conditions_description" value={formData.conditions_description} onChange={handleChange} rows={3} placeholder="Brief description of your condition and history…" />
          </Field>
          <Field label="Address *">
            <textarea style={{ ...styles.input, resize: 'vertical' }} name="address" value={formData.address} onChange={handleChange} required rows={2} placeholder="Your residential address" />
          </Field>
